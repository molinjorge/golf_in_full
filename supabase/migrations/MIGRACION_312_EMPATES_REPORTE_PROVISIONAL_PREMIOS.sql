...
-- MIGRACIÓN 312
-- Corrección de empates en el REPORTE PROVISIONAL EN LÍNEA
--
-- Antecedente:
--   La RPC obtener_reporte_provisional_premios_306() utilizaba RANK(), pero
--   incluía capturado_at e id dentro del ORDER BY competitivo. Eso provocaba
--   que dos resultados con el mismo valor recibieran posiciones distintas.
--
-- Corrección:
--   - MENOR_ES_MEJOR: RANK() únicamente por valor ASC.
--   - MAYOR_ES_MEJOR: RANK() únicamente por valor DESC.
--   - SOLO_REGISTRO: provisionalPosition permanece NULL.
--   - capturado_at e id se conservan únicamente para ordenar visualmente
--     registros empatados de forma determinística.
--
-- No modifica adjudicación oficial, motores deportivos ni ciclos del torneo.
-- ============================================================================
...
create or replace function public.obtener_reporte_provisional_premios_306(
    ...
    p_tournament_round_id uuid default null::uuid
)
returns jsonb
language plpgsql
security definer
