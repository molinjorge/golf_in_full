-- =========================================================
-- MIGRACIÓN 098
-- En torneos de categoría única, orden de prioridad:
-- 1) Mujeres -> siempre Rojas (gana sobre cualquier otra regla,
--    incluida edad senior).
-- 2) Hombres de edad senior -> siempre Doradas (Senior 1 y 2
--    comparten la misma marca aquí, no se distinguen entre sí).
-- 3) Hombres que no son senior -> franjas de hándicap (sin cambio).
--
-- El corte de edad "senior" usa por default el edad_minima más
-- bajo entre las categorías Senior del catálogo global, pero es
-- editable por torneo (tournaments.edad_senior_categoria_unica).
-- =========================================================

alter table tournaments
  add column edad_senior_categoria_unica integer;

comment on column tournaments.edad_senior_categoria_unica is 'Override, solo para este torneo, de la edad a partir de la cual un caballero sale de Doradas en categoría única. NULL = usa el edad_minima más bajo entre las categorías Senior del catálogo global.';
...
comment on function resolver_categoria_y_marca is 'Asigna marca_salida_id automáticamente. Categoría única: mujeres siempre Rojas (gana sobre senior); caballeros senior siempre Doradas; caballeros no-senior por franja de hándicap. Con varias categorías: valida/reasigna por hándicap o edad. Con equipo, hereda del equipo.';
