-- ============================================================
-- TEE CENTRAL / GOLF IN FULL
-- MIGRACIÓN 275
-- START_TOURNAMENT bloquea composición A-Go-Go no resuelta
--
-- Depende de Migración 274.
-- No resuelve ni modifica equipos. Sólo integra el detector 274
-- al contrato de estado y a la operación real iniciar_torneo().
-- ============================================================

CREATE OR REPLACE FUNCTION public.obtener_estado_inicio_torneo_256(
    p_tournament_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path TO 'public', 'pg_temp'
AS $function$
DECLARE
    v_t public.tournaments%ROWTYPE;
    v_round_id uuid;
    v_round_number integer;
    v_frozen boolean := false;
    v_starts_validated boolean := false;
    v_cards_issued boolean := false;
    v_cards integer := 0;
    v_sessions integer := 0;
    v_capture_ready boolean := false;
    v_errors jsonb := '[]'::jsonb;
    v_ready boolean := false;
    v_already_started boolean := false;

    v_format_code text;
    v_participation_type text;
    v_scoring_engine text;
    v_is_a_gogo_team boolean := false;
    v_composition jsonb := NULL;
    v_composition_ready boolean := true;
BEGIN
    SELECT *
      INTO v_t
      FROM public.tournaments
     WHERE id = p_tournament_id;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'El torneo indicado no existe.'
            USING ERRCODE = '22023';
    END IF;

    SELECT tf.code,
           tf.tipo_participacion::text,
           tf.scoring_engine::text
      INTO v_format_code,
           v_participation_type,
           v_scoring_engine
      FROM public.tournament_formats tf
     WHERE tf.id = v_t.tournament_format_id;

    v_is_a_gogo_team :=
        v_format_code = 'A_GOGO'
        AND v_participation_type = 'equipo'
        AND v_scoring_engine = 'team_stroke';

    v_already_started :=
        v_t.estatus IN (
            'en_curso'::public.estatus_torneo,
            'finalizado'::public.estatus_torneo
        );

    v_frozen := EXISTS (
        SELECT 1
        FROM public.tournament_condition_freezes f
        WHERE f.tournament_id = p_tournament_id
    );

    SELECT tr.id, tr.numero_ronda
      INTO v_round_id, v_round_number
      FROM public.tournament_rounds tr
     WHERE tr.tournament_id = p_tournament_id
       AND tr.activo = true
     ORDER BY tr.numero_ronda ASC, tr.fecha ASC, tr.id ASC
     LIMIT 1;

    IF v_round_id IS NOT NULL THEN
        v_starts_validated :=
            public._salida_ronda_esta_validada(v_round_id);

        v_cards_issued :=
            public._ronda_tiene_tarjetas_emitidas(v_round_id);

        SELECT count(*)
          INTO v_cards
          FROM public.tournament_score_cards sc
         WHERE sc.tournament_round_id = v_round_id
           AND sc.status = 'issued';

        SELECT count(*)
          INTO v_sessions
          FROM public.tournament_scorecard_capture_sessions cs
          JOIN public.tournament_score_cards sc
            ON sc.id = cs.score_card_id
         WHERE sc.tournament_round_id = v_round_id
           AND sc.status = 'issued';

        v_capture_ready := (v_cards > 0 AND v_sessions = v_cards);
    END IF;

    -- 275: sólo A-Go-Go TEAM consume el detector 274.
    IF v_is_a_gogo_team THEN
        v_composition :=
            public.obtener_estado_equipos_incompletos_a_gogo_274(
                p_tournament_id
            );

        v_composition_ready :=
            COALESCE(
                (v_composition->>'compositionReady')::boolean,
                false
            );
    END IF;

    -- Compatibilidad histórica: un torneo ya iniciado/finalizado
    -- no se vuelve a bloquear ni se reinterpreta competitivamente.
    IF v_already_started THEN
        RETURN jsonb_build_object(
            'tournamentId', p_tournament_id,
            'status', v_t.estatus::text,
            'alreadyStarted', true,
            'readyToStart', false,
            'firstRoundId', v_round_id,
            'firstRoundNumber', v_round_number,
            'conditionsFrozen', v_frozen,
            'startsValidated', v_starts_validated,
            'cardsIssued', v_cards_issued,
            'issuedCards', v_cards,
            'captureSessions', v_sessions,
            'captureReady', v_capture_ready,
            'teamCompositionApplicable', v_is_a_gogo_team,
            'teamCompositionReady', v_composition_ready,
            'teamComposition', v_composition,
            'errors', '[]'::jsonb
        );
    END IF;

    IF v_t.estatus <> 'inscripcion_cerrada'::public.estatus_torneo THEN
        v_errors := v_errors || jsonb_build_array(
            format(
                'El torneo sólo puede iniciarse desde INSCRIPCIÓN CERRADA. Estado actual: %s.',
                v_t.estatus
            )
        );
    END IF;

    IF NOT v_frozen THEN
        v_errors := v_errors || jsonb_build_array(
            'Primero deben congelarse las condiciones y hándicaps.'
        );
    END IF;

    IF v_round_id IS NULL THEN
        v_errors := v_errors || jsonb_build_array(
            'No existe una primera ronda activa.'
        );
    ELSE
        IF NOT v_starts_validated THEN
            v_errors := v_errors || jsonb_build_array(
                'Las salidas de la primera ronda aún no están validadas.'
            );
        END IF;

        IF NOT v_cards_issued THEN
            v_errors := v_errors || jsonb_build_array(
                'La primera ronda no tiene tarjetas oficiales emitidas.'
            );
        END IF;

        IF NOT v_capture_ready THEN
            v_errors := v_errors || jsonb_build_array(
                'La captura digital de la primera ronda no está completamente inicializada.'
            );
        END IF;
    END IF;

    IF v_is_a_gogo_team AND NOT v_composition_ready THEN
        v_errors := v_errors || jsonb_build_array(
            format(
                'La composición de equipos A-Go-Go tiene casos pendientes: %s equipo(s) incompleto(s), %s jugador(es) suelto(s) y %s jugador(es) activo(s) sin equipo.',
                COALESCE((v_composition->>'incompleteTeamCount')::integer,0),
                COALESCE((v_composition->>'loosePlayerCount')::integer,0),
                COALESCE((v_composition->>'unassignedActivePlayerCount')::integer,0)
            )
        );
    END IF;

    v_ready := jsonb_array_length(v_errors) = 0;

    RETURN jsonb_build_object(
        'tournamentId', p_tournament_id,
        'status', v_t.estatus::text,
        'alreadyStarted', false,
        'readyToStart', v_ready,
        'firstRoundId', v_round_id,
        'firstRoundNumber', v_round_number,
        'conditionsFrozen', v_frozen,
        'startsValidated', v_starts_validated,
        'cardsIssued', v_cards_issued,
        'issuedCards', v_cards,
        'captureSessions', v_sessions,
        'captureReady', v_capture_ready,
        'teamCompositionApplicable', v_is_a_gogo_team,
        'teamCompositionReady', v_composition_ready,
        'teamComposition', v_composition,
        'errors', v_errors
    );
END;
$function$;

CREATE OR REPLACE FUNCTION public.iniciar_torneo(
    p_tournament_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public', 'pg_temp'
AS $function$
DECLARE
    v_t public.tournaments%ROWTYPE;
    v_round_id uuid;
    v_round_number integer;
    v_cards integer := 0;
    v_sessions integer := 0;
    v_start_state jsonb;
BEGIN
    IF auth.uid() IS NULL THEN
        RAISE EXCEPTION 'No autenticado.'
            USING ERRCODE = '42501';
    END IF;

    IF NOT (
        public.is_superadmin(auth.uid())
        OR public.is_tournament_organizer(auth.uid(), p_tournament_id)
    ) THEN
        RAISE EXCEPTION
            'Sólo el organizador asignado o el Superadmin pueden iniciar el torneo.'
            USING ERRCODE = '42501';
    END IF;

    SELECT *
      INTO v_t
      FROM public.tournaments
     WHERE id = p_tournament_id
     FOR UPDATE;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'El torneo indicado no existe.'
            USING ERRCODE = '22023';
    END IF;

    IF v_t.estatus = 'en_curso'::public.estatus_torneo THEN
        RETURN jsonb_build_object(
            'ok', true,
            'tournamentId', p_tournament_id,
            'alreadyStarted', true,
            'estatus', 'en_curso'
        );
    END IF;

    -- 275: el mismo contrato que informa a la UI protege la
    -- operación real. Así no existe bypass por llamada directa.
    v_start_state :=
        public.obtener_estado_inicio_torneo_256(p_tournament_id);

    IF NOT COALESCE((v_start_state->>'readyToStart')::boolean,false) THEN
        RAISE EXCEPTION
            'No se puede iniciar el torneo porque existen requisitos pendientes.'
            USING ERRCODE = '23514',
                  DETAIL = COALESCE(
                      (v_start_state->'errors')::text,
                      '[]'
                  ),
                  HINT =
                      'Consulta el estado de inicio y resuelve los requisitos pendientes antes de volver a intentar.';
    END IF;

    v_round_id := NULLIF(v_start_state->>'firstRoundId','')::uuid;
    v_round_number := NULLIF(v_start_state->>'firstRoundNumber','')::integer;
    v_cards := COALESCE((v_start_state->>'issuedCards')::integer,0);
    v_sessions := COALESCE((v_start_state->>'captureSessions')::integer,0);

    PERFORM set_config(
        'app.permitir_cambio_estatus_torneo',
        '1',
        true
    );

    UPDATE public.tournaments
       SET estatus = 'en_curso'::public.estatus_torneo
     WHERE id = p_tournament_id;

    RETURN jsonb_build_object(
        'ok', true,
        'tournamentId', p_tournament_id,
        'alreadyStarted', false,
        'estatusAnterior', 'inscripcion_cerrada',
        'estatus', 'en_curso',
        'firstRoundId', v_round_id,
        'firstRoundNumber', v_round_number,
        'issuedCards', v_cards,
        'captureSessions', v_sessions,
        'teamCompositionApplicable',
            COALESCE(
                (v_start_state->>'teamCompositionApplicable')::boolean,
                false
            ),
        'teamCompositionReady',
            COALESCE(
                (v_start_state->>'teamCompositionReady')::boolean,
                true
            )
    );
END;
$function$;

COMMENT ON FUNCTION public.obtener_estado_inicio_torneo_256(uuid)
IS '256/259/275: contrato de preparación de START_TOURNAMENT; 275 integra composición A-Go-Go del detector 274.';

COMMENT ON FUNCTION public.iniciar_torneo(uuid)
IS '256/275: inicia el torneo sólo cuando el contrato de preparación está completo; 275 bloquea composición A-Go-Go no resuelta.';
