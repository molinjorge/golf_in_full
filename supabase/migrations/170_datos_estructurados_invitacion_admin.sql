-- ============================================================================
-- 170_datos_estructurados_invitacion_admin.sql
-- Tee Central / GOLF IN FULL
--
-- Agrega nombres y apellidos estructurados a las invitaciones administrativas
-- para que el invitado NO tenga que volver a capturarlos durante la activación.
--
-- Transición segura:
-- - No borra invitaciones existentes.
-- - Las nuevas invitaciones creadas con la nueva RPC guardan nombres/apellidos.
-- - La nueva aceptación de 1 parámetro exige que la invitación ya tenga esos datos.
-- - La firma anterior de aceptación se conserva temporalmente.
-- ============================================================================
...
'Nombres capturados por Superadmin al crear/asignar al usuario administrativo.';

COMMENT ON COLUMN public.admin_user_invitations.apellidos IS
'Apellidos capturados por Superadmin al crear/asignar al usuario administrativo.';

-- ============================================================================
-- NUEVA FIRMA CANÓNICA DE ALTA/ASIGNACIÓN
-- ============================================================================
...
BEGIN
  ...
  IF v_invitation_id IS NULL THEN
    INSERT INTO public.admin_user_invitations(
      display_name,nombres,apellidos,email,phone,
      role_id,club_id,tournament_id,status,invited_by
    )
    VALUES(
      v_display_name,v_nombres,v_apellidos,v_email,v_phone,
      v_role_id,p_club_id,p_tournament_id,
...
-- ============================================================================
-- ACEPTACIÓN NUEVA: SOLO invitation_id
-- ============================================================================
...
BEGIN
  ...
  IF NULLIF(trim(v_inv.nombres),'') IS NULL
     OR NULLIF(trim(v_inv.apellidos),'') IS NULL THEN
    RAISE EXCEPTION
      'La invitación requiere nombres y apellidos estructurados antes de poder activarse.'
      USING ERRCODE='55000';
  END IF;
  ...
  IF v_admin_id IS NULL THEN
    ...
    VALUES(
      v_auth_uid,v_auth_email,v_inv.nombres,v_inv.apellidos,true
    )
  ...
  IF v_assignment_id IS NULL THEN
    ...
    VALUES(
      v_admin_id,v_inv.role_id,v_inv.club_id,v_inv.tournament_id,
      v_inv.invited_by,true
    )
    RETURNING id INTO v_assignment_id;
  END IF;
  ...
  RETURN jsonb_build_object(
    'ok',true,'invitationId',v_inv.id,
    'adminUserId',v_admin_id,'assignmentId',v_assignment_id,
    'roleCode',v_role_code,'clubId',v_inv.club_id,
    'tournamentId',v_inv.tournament_id,
    'email',v_auth_email,'status','accepted'
  );
END $$;

-- Compatibilidad temporal: la firma antigua sigue existiendo,
-- pero la nueva UI debe usar aceptar_invitacion_admin(uuid).
CREATE OR REPLACE FUNCTION public.aceptar_invitacion_admin(
  p_invitation_id uuid,
  p_nombres text,
  p_apellidos text
...
SET search_path=public,pg_temp
AS $$
  SELECT public.aceptar_invitacion_admin(p_invitation_id);
$$;
