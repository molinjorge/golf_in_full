-- ============================================================================
-- 160_motor_desempates_ronda.sql
-- GOLF IN FULL / Tee Central
--
-- MIGRACIÓN 160 — MOTOR DE DESEMPATES DE RONDA
--
-- OBJETIVO
-- Evaluar de forma determinística, auditable y SOLO LECTURA los empates
-- existentes en una ronda, respetando la configuración YA EXISTENTE:
--
--   categoría + tipo_resultado + alcance -> secuencia efectiva
--
-- PRECEDENCIA DE CONFIGURACIÓN
-- Para cada grupo empatado:
--
-- 1. CATEGORY_SCOPE
--    Regla activa específica de categoría + alcance exacto.
--
-- 2. CATEGORY_ALL
--    Regla activa específica de categoría + alcance=todos.
--
-- 3. TOURNAMENT_SCOPE
--    Default del torneo (category NULL) + alcance exacto.
--
-- 4. TOURNAMENT_ALL
--    Default del torneo (category NULL) + alcance=todos.
--
-- IMPORTANTE:
-- - Se elige UNA sola fuente de secuencia.
-- - NO se mezclan pasos de override con pasos default.
--
-- TIPOS
-- - gross
-- - neto
--
-- ALCANCE
-- - empate en posición 1 -> primer_lugar
-- - cualquier otra posición -> otros_lugares
--
-- MÉTODOS AUTOMÁTICOS SOPORTADOS
-- - TARJETA_ULTIMOS_9
-- - TARJETA_ULTIMOS_6
-- - TARJETA_ULTIMOS_3
-- - TARJETA_ULTIMO_HOYO
...
--
-- Si se llega a un método no automático con el empate aún vivo:
--   status = MANUAL_PENDING
--
-- Si se agotan reglas automáticas y persiste el empate:
--   status = TIE_PERSISTS_AFTER_RULES
--
-- La Migración 160:
-- - NO modifica tournament_tiebreak_rules.
-- - NO materializa posiciones.
-- - NO modifica leaderboard 158.
-- - NO implementa premiación.
-- - NO implementa cortes/MC.
...
BEGIN
    IF to_regprocedure('public.obtener_resultados_oficiales_ronda(uuid)') IS NULL THEN
        RAISE EXCEPTION
            'Migración 160 requiere obtener_resultados_oficiales_ronda(uuid).';
    END IF;

    IF to_regprocedure('public.obtener_leaderboard_ronda(uuid)') IS NULL THEN
        RAISE EXCEPTION
            'Migración 160 requiere obtener_leaderboard_ronda(uuid).';
    END IF;

    IF to_regclass('public.tournament_tiebreak_rules') IS NULL
       OR to_regclass('public.tiebreak_methods') IS NULL
    THEN
        RAISE EXCEPTION
            'Migración 160 requiere tournament_tiebreak_rules y tiebreak_methods.';
    END IF;
END;
$$;

-- ============================================================================
-- 1. HELPER PURO — CLAVE DE UN MÉTODO PARA UNA TARJETA
--
-- Devuelve integer[] porque PostgreSQL compara arrays lexicográficamente.
-- Menor score = mejor desempate.
--
-- Para últimos 9/6/3/1 y tarjeta completa:
--   [suma]
--
-- Para hoyo por hoyo por hándicap:
--   [score_hcp1, score_hcp2, ..., score_hcpN]
--
-- Para métodos manuales/no soportados:
--   NULL
-- ============================================================================

CREATE OR REPLACE FUNCTION public.calcular_clave_metodo_desempate(
    p_method_code text,
    p_tipo_resultado tipo_resultado_desempate,
    p_holes jsonb
)
RETURNS integer[]
LANGUAGE plpgsql
...
BEGIN
    ...
    SELECT max((h->>'playSequence')::integer)
      INTO v_max_sequence
      FROM jsonb_array_elements(p_holes) h;

    IF v_max_sequence IS NULL OR v_max_sequence <= 0 THEN
        RETURN NULL;
    END IF;
    ...
    IF v_code IN (
        ...
        v_take := LEAST(v_take, v_max_sequence);
        v_start_sequence := v_max_sequence - v_take + 1;
        ...
        SELECT
            ...
            ARRAY[
                sum(
                    CASE
                        WHEN p_tipo_resultado='gross'::tipo_resultado_desempate
          ...
          FROM jsonb_array_elements(p_holes) h
         WHERE (h->>'playSequence')::integer
               BETWEEN v_start_sequence AND v_max_sequence
           AND (
                CASE
                    WHEN p_tipo_resultado='gross'::tipo_resultado_desempate
...
-- ============================================================================
-- 2. HELPER PURO — EVALUAR UNA SECUENCIA PARA UNA TARJETA
--
-- p_rules esperado:
-- [
--   {
...
-- ]
--
-- Devuelve evidencia por paso y clave acumulada.
-- Se detiene en el primer método no automático.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.evaluar_secuencia_desempate_tarjeta(
    p_rules jsonb,
    p_tipo_resultado tipo_resultado_desempate,
    p_holes jsonb
)
...
BEGIN
    ...
    LOOP
        ...
        v_key := public.calcular_clave_metodo_desempate(
            v_method_code,
            p_tipo_resultado,
            p_holes
        );
...
-- ============================================================================
-- 3. RPC — MOTOR DE DESEMPATES DE RONDA
-- ============================================================================
...
BEGIN
    ...
    IF NOT public.puede_administrar_congelamiento_torneo(v_tournament_id) THEN
        RAISE EXCEPTION
            'No tienes permiso administrativo para consultar desempates.'
            USING ERRCODE='42501';
    END IF;
    ...
    v_leaderboard := public.obtener_leaderboard_ronda(
        p_tournament_round_id
    );

    v_round_resolved := COALESCE(
        (v_leaderboard->'status'->>'roundResolved')::boolean,
        false
    );
    ...
    RETURN (
        WITH official_cards AS (
            SELECT
                ...
                (c->>'officialGrossTotal')::integer
                    AS official_gross_total,
                (c->>'officialNetTotal')::integer
        ...
        metric_rows AS (
            SELECT
                oc.*,
                'gross'::tipo_resultado_desempate AS result_type,
                oc.official_gross_total AS metric_total
            ...
            SELECT
                oc.*,
                'neto'::tipo_resultado_desempate AS result_type,
                oc.official_net_total AS metric_total
            FROM official_cards oc
        ),
        ...
        tied AS (
            SELECT
                ...
                CASE
                    WHEN r.base_rank=1
                        THEN 'primer_lugar'::alcance_desempate
                    ELSE 'otros_lugares'::alcance_desempate
                END AS scope
            FROM ranked r
            WHERE r.tie_size>1
        ),
                ...
                CASE
                    ...
                    WHEN EXISTS (
                        ...
                        WHERE r.tournament_id=v_tournament_id
                          ...
                          AND r.tipo_resultado=g.result_type
                          AND r.tournament_category_id=g.tournament_category_id
                          AND r.alcance='todos'::alcance_desempate
                    ) THEN 'CATEGORY_ALL'
                ...
                SELECT jsonb_agg(
                    jsonb_build_object(
                        ...
                        COALESCE((
                            ...
                            WHERE pr.tournament_category_id=
                                  gf.tournament_category_id
...
REVOKE ALL ON FUNCTION public.calcular_clave_metodo_desempate(
    text,tipo_resultado_desempate,jsonb
) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.calcular_clave_metodo_desempate(
    text,tipo_resultado_desempate,jsonb
) TO authenticated;

REVOKE ALL ON FUNCTION public.evaluar_secuencia_desempate_tarjeta(
    jsonb,tipo_resultado_desempate,jsonb
) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.evaluar_secuencia_desempate_tarjeta(
    jsonb,tipo_resultado_desempate,jsonb
) TO authenticated;

REVOKE ALL ON FUNCTION public.obtener_desempates_ronda(uuid)
FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.obtener_desempates_ronda(uuid)
TO authenticated;
