-- 128_asignar_categoria_al_resolver_franjas.sql
--
-- Ajuste visual pedido por el usuario: al resolver la marca de salida por franjas
-- (categoría única), la inscripción se quedaba sin tournament_category_id — funciona
-- bien (la marca no depende de tener categoría en este modelo), pero se ve
-- inconsistente en la pantalla "Jugadores inscritos": algunos jugadores muestran
-- "UNICA" en la columna Categoría (por haberla recibido explícitamente en otro punto
-- del flujo) y otros la muestran vacía (los resueltos solo por franjas, como los
-- jugadores cortesía de patrocinador).
--
-- Fix: al resolver por franjas, se asigna también la categoría del torneo (se asume
-- el modelo estándar de categoría única: una sola fila en tournament_categories para
-- ese torneo). Si por alguna razón hubiera más de una, se toma la primera — este
-- modelo no está diseñado para múltiples categorías conviviendo con franjas.
...
begin
  ...
  if new.tournament_team_id is null or v_categoria_estandar is null then
    ...
    if v_hay_franjas then
      ...
      -- NUEVO: asigna la categoría del torneo (asume categoría única) para
      -- consistencia visual, sin afectar la lógica de marca ya resuelta arriba.
      if new.tournament_category_id is null then
        select id into v_categoria_unica_id
