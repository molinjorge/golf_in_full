-- ============================================================================
-- 174_fase3_resultados_oficiales_desde_snapshots.sql
-- Tee Central / GOLF IN FULL
--
-- MIGRACIÓN 174 — FASE 3
-- RESULTADOS OFICIALES SIN DEPENDENCIA DE CAPTURA DIGITAL
--
-- REGLA FUNCIONAL:
-- - Tarjeta física: OBLIGATORIA.
-- - Captura digital: OPCIONAL.
-- - Los hoyos esperados provienen de tournament_round_hole_snapshots.
-- - Físico sin digital = PHYSICAL_ONLY y puede ser oficial.
-- - Diferencias/disputas pendientes siguen bloqueando si no hay resolución.
--
-- ALCANCE:
-- - Reemplaza obtener_resultados_oficiales_ronda(uuid).
-- - No modifica leaderboard: éste seguirá consumiendo resultados oficiales.
-- - No crea sesiones digitales ni hole_scores ficticios.
-- ============================================================================
...
BEGIN
    ...
    RETURN (
        ...
        round_shape AS (
            SELECT
                count(*)::integer AS holes_expected
            FROM public.tournament_round_hole_snapshots rh
            WHERE rh.tournament_round_id = p_tournament_round_id
        ),
        ...
        cards AS (
            SELECT
                ...
                hsnap.handicap_index,
                hsnap.handicap_source,
                hsnap.handicap_status,
                hsnap.tee_name AS handicap_tee_name,
            ...
            LEFT JOIN public.tournament_handicap_snapshots hsnap
              ON hsnap.id = u.handicap_snapshot_id

            LEFT JOIN public.tournament_round_handicap_snapshots rhs
              ON rhs.id = u.round_handicap_snapshot_id

            CROSS JOIN round_shape rs
        ...
        evidence AS (
            SELECT
                ...
                dhs.gross_score AS digital_gross_score,
                dhs.status AS digital_status,
                ...
                CASE
                    ...
                    WHEN dhs.gross_score IS NULL
                        THEN 'SIN_CAPTURA_DIGITAL'
                    WHEN dhs.gross_score = phs.physical_gross_score
                        THEN 'COINCIDE'
                    ELSE 'DIFERENCIA'
                ...
                CASE
                    ...
                    WHEN dhs.gross_score IS NULL
                        ...
                        THEN 'PHYSICAL_ONLY'
                    WHEN dhs.gross_score IS NOT NULL
                         AND dhs.gross_score = phs.physical_gross_score
                        THEN 'MATCHED'
            ...
            JOIN public.tournament_round_hole_snapshots rh
              ON rh.tournament_round_id = c.tournament_round_id

            LEFT JOIN public.tournament_scorecard_hole_scores dhs
              ON dhs.score_card_id = c.score_card_id
             AND dhs.round_hole_snapshot_id = rh.id

            LEFT JOIN public.tournament_scorecard_physical_hole_scores phs
              ON phs.score_card_id = c.score_card_id
             AND phs.round_hole_snapshot_id = rh.id

            LEFT JOIN public.tournament_scorecard_hole_resolutions res
              ON res.score_card_id = c.score_card_id
             AND res.round_hole_snapshot_id = rh.id

            LEFT JOIN last_dispute ld
              ON ld.hole_score_id = dhs.id

            LEFT JOIN last_confirm lc
              ON lc.hole_score_id = dhs.id
        ),
        ...
        shape AS (
            SELECT
                ...
                count(cl.round_hole_snapshot_id) AS hole_rows,
        ...
        readiness AS (
            SELECT
                ...
                c.holes_expected AS expected_holes,
                ...
                (
                    ...
                    AND c.holes_expected > 0
                    AND s.hole_rows = c.holes_expected
                    AND s.physical_holes = c.holes_expected
                    AND s.official_holes = c.holes_expected
                    AND s.invalid_official_source = 0
                    AND s.unresolved_review_holes = 0
                    AND s.distinct_si = c.holes_expected
                    AND s.min_si = 1
                    AND s.max_si = c.holes_expected
                ...
                CASE
                    ...
                    WHEN c.physical_status = 'CAPTURED'
                         AND c.reconciliation_status = 'COMPLETED'
                         AND (
                            s.distinct_si <> c.holes_expected
                            OR s.min_si <> 1
                            OR s.max_si <> c.holes_expected
                         )
                        THEN 'STROKE_INDEX_INVALID'
                    WHEN c.physical_status = 'CAPTURED'
                         AND c.reconciliation_status = 'COMPLETED'
                         AND (
                            s.hole_rows <> c.holes_expected
                            OR s.physical_holes <> c.holes_expected
                            OR s.official_holes <> c.holes_expected
            ...
            SELECT
                ...
                CASE
                    WHEN r.structurally_ready
                        ...
                        THEN 'OFFICIAL_READY'
                    WHEN r.structurally_ready
                         AND t.handicap_strokes_total
                             IS DISTINCT FROM r.playing_handicap
            ...
            COALESCE((
                SELECT jsonb_agg(
                    jsonb_build_object(
                        ...
                        'holesOfficial',
                            CASE
                                WHEN rr.ready THEN rr.official_holes
                                ELSE NULL
                            END,

                        'officialGrossTotal',
                            CASE
                                WHEN rr.ready THEN rr.official_gross_total
                                ELSE NULL
                            END,

                        'handicapStrokesTotal',
                            CASE
                                WHEN rr.ready THEN rr.handicap_strokes_total
                                ELSE NULL
                            END,

                        'officialNetTotal',
                            CASE
                                WHEN rr.ready THEN rr.official_net_total
                                ELSE NULL
                            END,
                            ...
                            CASE
                                WHEN rr.ready THEN COALESCE((
                                    SELECT jsonb_agg(
                                        ...
                                        ORDER BY hc.play_sequence, hc.hole_number
                                    )
                                    FROM hole_calc hc
