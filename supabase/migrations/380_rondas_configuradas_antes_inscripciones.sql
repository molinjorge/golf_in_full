...
-- MIGRACIÓN 380
-- RONDAS REALMENTE CONFIGURADAS ANTES DE ABRIR INSCRIPCIONES
-- ============================================================================
-- Objetivo:
--   Corregir el criterio roundStructureReady introducido en 379.
--   Una ronda no está lista sólo por existir y estar activa: antes de abrir
--   inscripciones debe tener su configuración básica terminada.
--
-- Regla:
--   Cada ronda declarada (1..numero_rondas) debe:
--     * existir y estar activa;
--     * tener fecha;
--     * tener campo_golf_id;
--     * usar el mismo campo del torneo;
--     * tener formato_salida.
--
-- No exige todavía grupos, horarios/salidas materializadas, validación de
-- salidas, emisión de tarjetas ni captura. Esos pasos siguen siendo posteriores.
-- ============================================================================

BEGIN;

CREATE OR REPLACE FUNCTION public._estado_apertura_inscripciones_379(
...
BEGIN
        ...
        SELECT COALESCE(
                   jsonb_agg(
                       jsonb_build_object(
                           ...
                           'date',r.fecha,
                           'courseId',r.campo_golf_id,
                           'startFormat',r.formato_salida,
                           'missing',
                               (SELECT jsonb_agg(x)
                                  FROM unnest(ARRAY[
         ...
         WHERE r.tournament_id=p_tournament_id
           ...
           AND (
               r.fecha IS NULL
               OR r.campo_golf_id IS NULL
               OR r.campo_golf_id IS DISTINCT FROM v_t.campo_golf_id
               OR r.formato_salida IS NULL
           );
    END IF;
    ...
    v_rounds_ready :=
        ...
        AND v_active=v_declared
        AND jsonb_array_length(v_unconfigured)=0;
    ...
    RETURN jsonb_build_object(
        ...
        'unconfiguredRounds',v_unconfigured,
...
'380: apertura de inscripciones exige rondas existentes, activas y configuradas (fecha, campo del torneo y formato de salida).';
