-- ============================================================================
-- 155_resultados_oficiales_masivos_ronda.sql
-- GOLF IN FULL / Tee Central
--
-- MIGRACIÓN 155 — FUENTE MASIVA DE RESULTADOS OFICIALES POR RONDA
--
-- OBJETIVO
-- Entregar, en UNA SOLA RPC y SIN N+1 desde frontend, el estado y resultado
-- oficial de TODAS las tarjetas emitidas de una ronda.
--
-- Para cada tarjeta devuelve:
-- - jugador / categoría / tee;
-- - Handicap Index / Course Handicap / Playing Handicap;
-- - estado operativo de físico + conciliación;
-- - GROSS oficial total;
-- - golpes de hándicap distribuidos;
-- - NETO oficial total;
-- - detalle por hoyo cuando el resultado está listo;
-- - issues estructurales cuando todavía no puede considerarse oficial.
--
-- PRINCIPIOS
-- - NO llama una RPC por tarjeta.
-- - NO depende de categoría para decidir GROSS vs NETO.
-- - GROSS y NETO se calculan siempre que la tarjeta esté lista.
-- - NO consulta hándicap vivo.
-- - NO materializa resultados.
-- - NO asigna posiciones.
-- - NO implementa leaderboard, premios, cortes ni desempates.
-- ============================================================================
...
BEGIN
    IF to_regprocedure(
        'public.calcular_golpes_handicap_hoyo(integer,integer,integer)'
    ) IS NULL THEN
        RAISE EXCEPTION
            'Migración 155 requiere public.calcular_golpes_handicap_hoyo(...) de la Migración 154.';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migración 155 requiere scorecards, físico, conciliación y snapshots de ronda.';
    END IF;

    IF to_regprocedure('public.puede_administrar_congelamiento_torneo(uuid)') IS NULL THEN
        RAISE EXCEPTION
            'Migración 155 requiere public.puede_administrar_congelamiento_torneo(uuid).';
    END IF;
END;
$$;
