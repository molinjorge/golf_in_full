-- MIGRACIÓN 270
-- Corrige la RPC 269 para permitir el flujo auditado cuando ya existen
-- tarjetas oficiales emitidas.
--
-- NO ejecutar automáticamente. Aplicar manualmente.
--
-- Diagnóstico:
-- trg_impedir_reapertura_con_tarjetas_emitidas exige
-- app.revisar_tarjeta_team_post_emision=true cuando una validación pasa de
-- validated a reopened y la ronda ya tiene tarjetas emitidas.
--
-- Los flujos post-emisión existentes (sustitución/reasignación) ya activan
-- ese guard. La RPC 269 activaba únicamente
-- app.reabrir_validacion_salida_ronda=true.
--
-- Alcance:
-- - Sólo reemplaza corregir_handicap_jugador_torneo_a_gogo_269.
-- - No modifica triggers.
-- - No modifica tablas.
-- - No modifica snapshots.
-- - No modifica _revisar_tarjetas_team_ronda_post_emision_215.
-- - No modifica frontend.
-- - El permiso interno se activa sólo si la ronda ya tiene tarjetas emitidas
--   y sólo dentro de la transacción actual.
...
BEGIN
    ...
    LOOP
        ...
        IF v_old_validation.id IS NOT NULL THEN
            ...
            -- Si ya existen tarjetas oficiales, habilitar el MISMO guard
            -- auditado que utilizan los flujos post-emisión 215.
            -- Es local a esta transacción (is_local=true).
...
-- Preservar ACL explícita de la RPC.
